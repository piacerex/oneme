...
VRMは実メッシュGLBへVRM 1.0のhumanoidノード、skin（JOINTS/WEIGHTS）、表情モーフ、`VRMC_springBone`を付与した`.vrm`として出力します。`/model-viewer.html?model_url=/exports/.../avatar.vrm`で、three-vrmを使ったWeb表示とVRM拡張の読み込みを確認できます。外部VRMビューアとUnityでの互換性は引き続き検証工程です。
...
`GET /api/monitoring/cdn`は環境変数`ONEME_CDN_URLS`にカンマ区切りで設定したHTTPSエンドポイントをHEADリクエストで検査します。`ONEME_CDN_ALLOWED_HOSTS`を設定すると対象ホストを制限でき、`ONEME_CDN_CHECK_INTERVAL_MS`で周期チェック間隔を変更できます。応答には現在プローブのSLO評価とアラートpayloadが含まれます。`ONEME_CDN_SLO_AVAILABILITY_PERCENT`と`ONEME_CDN_SLO_MAX_RESPONSE_MS`で目標を設定できます。

周期プローブは`monitoring_probe_runs`へ、ステータス、エンドポイント数、稼働数、アラートコードだけを保存し、URLやクエリ値は保存しません。応答の`historicalSlo`で直近プローブの稼働率集計を確認できます。保存履歴は内部集計用で、外部監視サービスのSLO・通知ポリシー・オンコール連携は別途接続します。

周期監視から外部通知へ接続する場合は`ONEME_MONITORING_ALERT_URL=https://...`と`ONEME_MONITORING_ALERT_SECRET=...`を設定します。HTTPSの通知先へ`oneme.monitoring`イベントを署名付きJSONでPOSTします。外部監視サービス側のSLO集計・通知ポリシーは本番導入時に接続します。

通知は`x-oneme-monitoring-event-id`と`idempotency-key`を付け、408／425／429／5xxまたは接続障害だけを同じイベントIDで限定再試行します。再試行回数・待機時間・タイムアウトは`ONEME_MONITORING_NOTIFY_MAX_ATTEMPTS`、`ONEME_MONITORING_NOTIFY_RETRY_DELAY_MS`、`ONEME_MONITORING_NOTIFY_TIMEOUT_MS`、`ONEME_MONITORING_NOTIFY_CONNECT_TIMEOUT_MS`で調整できます。localhost開発時だけ`ONEME_MONITORING_ALLOW_INSECURE_HTTP=true`を使えます。

利用規約とプライバシーポリシーの技術初稿は`docs/legal/terms.md`と`docs/legal/privacy.md`にあります。公開前の法務レビューで、事業者情報、提供地域、料金、保持期間、問い合わせ窓口を確定してください。

`POST /api/generation-jobs` は現在のアバター設定から3つの候補を作り、`POST /api/generation-jobs/:id/feedback` で採用・却下を記録します。認証済みチームではプランの`generation_requested`上限と契約状態を確認し、上限超過は429、`past_due`／`canceled`は402で停止します。現段階のプロバイダーは既存パーツを推薦するローカル実装で、画像生成サービスへ差し替えられるジョブ境界を使います。

顔写真マッピングでは、MediaPipe Face Landmarkerを利用できるブラウザで左右の目・鼻・口を正規顔座標へ再投影し、ロール・ヨー由来の左右差・ピッチ由来の縦ずれを2D正面化します。額・頬・顎の輪郭から卵型ヘッドメッシュを作り、写真はその正面半球だけへ貼り、横・背面へそのまま巻き付けません。`ONEME_OPENAI_API_KEY`を設定すると、画面の「側面・背面をAI補完」ボタンから、同意済みのキャリブレーション済み正面テクスチャだけを`POST /api/face-completion`へ送り、生成された一時的な横・背面アトラスを背面半球へ表示できます。キー未設定時は暫定陰影を表示しますが、AI推測ではありません。元写真と補完画像はサーバーへ保存しません。詳細と制約は`docs/face-calibration-and-profile-completion.md`を参照してください。

外部候補プロバイダーを使う場合は`ONEME_GENERATION_PROVIDER=http_json`、`ONEME_GENERATION_PROVIDER_URL=https://...`、必要なら`ONEME_GENERATION_PROVIDER_API_KEY=...`を設定します。送信JSONは`kind`とサニタイズ済み`avatarConfig`だけで、元写真のdata URLや画像バイナリは含みません。応答は`{"provider":"...","candidates":[...]}`契約で、候補の`parts`とHTTPSの`imageUrl`だけを保存します。

外部プロバイダーの応答に`moderation: {"status":"passed","provider":"..."}`と
`usage: {"inputTokens": 0, "outputTokens": 0, "costCents": 0}`を含めると、値を限定的に
正規化して`generation_provider_usage`利用イベントへ記録します。公開環境では
`ONEME_GENERATION_REQUIRE_MODERATION=true`を設定し、審査結果がない応答を拒否してください。
`blocked`、`rejected`、`failed`の審査結果は候補として保存しません。契約の詳細は
`docs/generation-provider-contract.md`にあります。
...
WSLやコンテナの外から開発画面を確認する場合は、`ONEME_BIND=0.0.0.0 mix phx.server`で開発用HTTPバインドを変更できます。既定値は`127.0.0.1`です。
