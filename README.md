...
顔写真マッピングでは、MediaPipe Face Landmarkerを利用できるブラウザで目の傾きを補正し、左右の目・鼻・口・顎を基準に正面テクスチャをキャリブレーションします。写真は顔の正面半球だけへ貼り、横・背面へそのまま巻き付けません。`ONEME_OPENAI_API_KEY`を設定すると、画面の「側面・背面をAI補完」ボタンから、同意済みのキャリブレーション済み正面テクスチャだけを`POST /api/face-completion`へ送り、生成された一時的な横・背面アトラスを背面半球へ表示できます。元写真と補完画像はサーバーへ保存しません。詳細と制約は`docs/face-calibration-and-profile-completion.md`を参照してください。
